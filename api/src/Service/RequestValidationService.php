...
class RequestValidationService
{
     ...
     *
     * @return array{success: bool, data?: array<int|string, mixed>, error?: string}
     */
    ...
    public function validateVehicleUpdateData(array $data): array
    {
        ...
        if (isset($data['brand'])) {
            if (!\is_string($data['brand']) || mb_strlen($data['brand']) > 100) {
                $errors['brand'] = 'Brand must be a string and cannot be longer than 100 characters';
            }
        }

        if (isset($data['model'])) {
            if (!\is_string($data['model']) || mb_strlen($data['model']) > 100) {
    ...
    private function validateFilters(array $filters): array
    {
        ...
        if (isset($filters['brand']) && \is_string($filters['brand']) && mb_strlen($filters['brand']) > 100) {
            $errors['brand'] = 'Brand filter cannot be longer than 100 characters';
        }

        if (isset($filters['model']) && \is_string($filters['model']) && mb_strlen($filters['model']) > 100) {
    ...
    private function validateOptionalVehicleFields(array $data): array
    {
        ...
        if (isset($data['color']) && (!\is_string($data['color']) || mb_strlen($data['color']) > 30)) {
            $errors['color'] = 'Color must be a string and cannot be longer than 30 characters';
        }

        if (isset($data['category']) && (!\is_string($data['category']) || mb_strlen($data['category']) > 50)) {
